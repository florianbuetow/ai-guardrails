...
post_baseline_tests() {
    ...
    for entry in "${injections[@]}"; do
        ...
        expected_id="${entry#*:}"
        violation_dir="$REPO_ROOT/violations/python-cli-base/$violation_name"
    ...
    backup_dir="$(mktemp -d)"
    inject_violation "$REPO_ROOT/violations/python-cli-base/no-default-values" "$project_dir" "$backup_dir"
    (cd "$project_dir" && git add -A)
    if score_output="$(cd "$project_dir" && UV_EXCLUDE_NEWER="2099-01-01" just score 2>&1)"; then
        ...
        restore_violation "$REPO_ROOT/violations/python-cli-base/no-default-values" "$project_dir" "$backup_dir"
        return 1
    fi
    restore_violation "$REPO_ROOT/violations/python-cli-base/no-default-values" "$project_dir" "$backup_dir"
    (cd "$project_dir" && git reset -q --hard HEAD && git clean -qfd)
