...
COPIER_DATA=(
    ...
    "project_description=Test Scala CLI application"
    "jvm_target=21"
...
post_baseline_tests() {
    ...
    if ! printf "%s\n" "$output" | grep -F "Hello, Scala!" >/dev/null; then
        log_fail "just run did not execute the dummy Scala main application"
        printf "%s\n" "$output"
        return 1
    fi
