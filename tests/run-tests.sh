...
usage() {
    printf "Usage: %s <python|java|go|elixir|cpp|cpp-3dgame|rust|kotlin|scala|clojure|typescript|mcp-typescript|all> [baseline]\n" "$0"
...
case "$1" in
    python|java|go|elixir|cpp|cpp-3dgame|rust|kotlin|scala|clojure|typescript|mcp-typescript)
        languages=("$1")
        ;;
    all)
        languages=(python java go elixir cpp cpp-3dgame rust kotlin scala clojure typescript mcp-typescript)
        ;;
    *)
        usage
        exit 1
