...
usage() {
    printf "Usage: %s <python|java|go|elixir|cpp|rust|kotlin|typescript|all> [baseline]\n" "$0"
...
case "$1" in
    python|java|go|elixir|cpp|rust|kotlin|typescript)
        languages=("$1")
        ;;
    all)
        languages=(python java go elixir cpp rust kotlin typescript)
        ;;
    *)
        usage
        exit 1
