...
usage() {
    printf "Usage: %s <python|java|go|elixir|cpp|cpp-3dgame|rust|kotlin|typescript|all> [baseline]\n" "$0"
...
case "$1" in
    python|java|go|elixir|cpp|cpp-3dgame|rust|kotlin|typescript)
        languages=("$1")
        ;;
    all)
        languages=(python java go elixir cpp cpp-3dgame rust kotlin typescript)
        ;;
    *)
        usage
        exit 1
